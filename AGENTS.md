...
│   │   └── SKILL.md              # Setup + routing for Claude Code, Codex CLI, Cursor, Gemini CLI, Antigravity CLI monitoring (~500 lines)
...
olakai agents mine [--source claude-code|codex|cursor|gemini-cli|antigravity] [--json]   # account-wide: your coding agents (>= 0.7.0)
...
# Gemini CLI (~/.gemini/settings.json), so the two coexist cleanly.
# Because Codex/Cursor/Gemini CLI/Antigravity hooks are global, their hook fires in workspaces with no .olakai config
